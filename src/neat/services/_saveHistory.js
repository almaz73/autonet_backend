...
export async function _saveHistory() {
...
async function saveNewCarsForThisDay() {
    ...
    const result = [...new Set([...arrFromBD, ...arrFromFile])];
    ...
    return `В историю БД добавлено новых: ${result.length - arrFromBD.length}. Всего за сегодня: ${count}. `
}

async function saveDeletedCarsForThisDay() {
    let deletedLinksFromFile = []
    const filePath = path.join(FolderLINKS, 'links_deletedToday_cars.js'); // вытаскивание по дате
    const fileContent = fs.readFileSync(filePath, 'utf8');
    const timeUpdateFile = fs.statSync(filePath);
