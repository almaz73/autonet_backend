export async function _clearBadPhotos(db) {
    try {
        // language=SQLite
        ...
        if(deleted && badFields.length) return ` Удалены плохие фото-ссылки на ${badFields.length} авто (kopiya, pdf)`
        else return ' Плохих фото-ссылок нет'
    } catch (error) {
        throw error;
    }
